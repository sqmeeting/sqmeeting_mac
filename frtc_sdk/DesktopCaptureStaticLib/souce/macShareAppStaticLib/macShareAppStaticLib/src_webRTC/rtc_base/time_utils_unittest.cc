...
TEST(FakeClock, TimeFunctionsUseFakeClock) {
  ...
  clock.SetTime(webrtc_mac_capturer::Timestamp::Micros(987654));
...
TEST(FakeClock, SetTime) {
  FakeClock clock;
  clock.SetTime(webrtc_mac_capturer::Timestamp::Micros(123));
  EXPECT_EQ(123000, clock.TimeNanos());
  clock.SetTime(webrtc_mac_capturer::Timestamp::Micros(456));
  EXPECT_EQ(456000, clock.TimeNanos());
}

TEST(FakeClock, AdvanceTime) {
  FakeClock clock;
  clock.AdvanceTime(webrtc_mac_capturer::TimeDelta::Micros(1u));
  EXPECT_EQ(1000, clock.TimeNanos());
  clock.AdvanceTime(webrtc_mac_capturer::TimeDelta::Micros(2222u));
  EXPECT_EQ(2223000, clock.TimeNanos());
  clock.AdvanceTime(webrtc_mac_capturer::TimeDelta::Millis(3333u));
  EXPECT_EQ(3335223000, clock.TimeNanos());
  clock.AdvanceTime(webrtc_mac_capturer::TimeDelta::Seconds(4444u));
...
TEST(FakeClock, SettingTimeWakesThreads) {
  ...
  worker->PostDelayedTask(webrtc_mac_capturer::ToQueuedTask([&message_handler_dispatched] {
  ...
  clock.AdvanceTime(webrtc_mac_capturer::TimeDelta::Seconds(60u));
