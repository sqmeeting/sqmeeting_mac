...
absl::optional<int> GetFallbackMinBpsFromFieldTrial(VideoCodecType type) {
  ...
  if (!webrtc_mac_capturer::field_trial::IsEnabled(kForcedFallbackFieldTrial)) {
    return absl::nullopt;
  }

  const std::string group =
      webrtc_mac_capturer::field_trial::FindFullName(kForcedFallbackFieldTrial);
  if (group.empty()) {
    return absl::nullopt;
  }
...
absl::optional<DataRate> GetExperimentalMinVideoBitrate(VideoCodecType type) {
  ...
  if (webrtc_mac_capturer::field_trial::IsEnabled(kMinVideoBitrateExperiment)) {
    webrtc_mac_capturer::FieldTrialFlag enabled("Enabled");

    // Backwards-compatibility with an old experiment - a generic minimum which,
    // if set, applies to all codecs.
    webrtc_mac_capturer::FieldTrialOptional<webrtc_mac_capturer::DataRate> min_video_bitrate("br");

    // New experiment - per-codec minimum bitrate.
    webrtc_mac_capturer::FieldTrialOptional<webrtc_mac_capturer::DataRate> min_bitrate_vp8("vp8_br");
    webrtc_mac_capturer::FieldTrialOptional<webrtc_mac_capturer::DataRate> min_bitrate_vp9("vp9_br");
    webrtc_mac_capturer::FieldTrialOptional<webrtc_mac_capturer::DataRate> min_bitrate_av1("av1_br");
    webrtc_mac_capturer::FieldTrialOptional<webrtc_mac_capturer::DataRate> min_bitrate_h264("h264_br");

    webrtc_mac_capturer::ParseFieldTrial(
        {&enabled, &min_video_bitrate, &min_bitrate_vp8, &min_bitrate_vp9,
         &min_bitrate_av1, &min_bitrate_h264},
        webrtc_mac_capturer::field_trial::FindFullName(kMinVideoBitrateExperiment));
