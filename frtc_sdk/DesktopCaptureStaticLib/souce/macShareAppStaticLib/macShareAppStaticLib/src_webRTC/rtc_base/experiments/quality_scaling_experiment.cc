...
#if defined(WEBRTC_IOS)
  return webrtc_mac_capturer::field_trial::IsEnabled(kFieldTrial);
#else
  return !webrtc_mac_capturer::field_trial::IsDisabled(kFieldTrial);
#endif
}

absl::optional<QualityScalingExperiment::Settings>
QualityScalingExperiment::ParseSettings() {
  std::string group = webrtc_mac_capturer::field_trial::FindFullName(kFieldTrial);
