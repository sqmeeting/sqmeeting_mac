...
using ::webrtc_mac_capturer::ToQueuedTask;
...
class AtomicBool {
 public:
  ...
  AtomicBool& operator=(bool value) {
    webrtc_mac_capturer::MutexLock scoped_lock(&mutex_);
    flag_ = value;
    return *this;
  }
  bool get() const {
 ...
 private:
  mutable webrtc_mac_capturer::Mutex mutex_;
  bool flag_;
};
...
TEST(ThreadTest, ThreeThreadsInvoke) {
  ...
  class LockedBool {
   public:
    ...
    void Set(bool value) {
      webrtc_mac_capturer::MutexLock lock(&mutex_);
      value_ = value;
    }
   ...
   private:
    webrtc_mac_capturer::Mutex mutex_;
...
TEST(ThreadPostDelayedTaskTest, InvokesInDelayOrder) {
  ...
  clock.AdvanceTime(webrtc_mac_capturer::TimeDelta::Millis(11));
  EXPECT_TRUE(fourth.Wait(0));
}

TEST(ThreadPostDelayedTaskTest, IsCurrentTaskQueue) {
  auto current_tq = webrtc_mac_capturer::TaskQueueBase::Current();
  {
    std::unique_ptr<rtc::Thread> thread(rtc::Thread::Create());
    thread->WrapCurrent();
    EXPECT_EQ(webrtc_mac_capturer::TaskQueueBase::Current(),
              static_cast<webrtc_mac_capturer::TaskQueueBase*>(thread.get()));
    thread->UnwrapCurrent();
  }
  EXPECT_EQ(webrtc_mac_capturer::TaskQueueBase::Current(), current_tq);
}

class ThreadFactory : public webrtc_mac_capturer::TaskQueueFactory {
 public:
  std::unique_ptr<webrtc_mac_capturer::TaskQueueBase, webrtc_mac_capturer::TaskQueueDeleter>
  CreateTaskQueue(absl::string_view /* name */,
                  Priority /*priority*/) const override {
    std::unique_ptr<Thread> thread = Thread::Create();
    thread->Start();
    return std::unique_ptr<webrtc_mac_capturer::TaskQueueBase, webrtc_mac_capturer::TaskQueueDeleter>(
        thread.release());
  }
};
