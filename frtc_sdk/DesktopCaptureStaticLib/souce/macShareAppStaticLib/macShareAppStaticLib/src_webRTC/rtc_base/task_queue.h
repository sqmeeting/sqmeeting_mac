...
class RTC_LOCKABLE RTC_EXPORT TaskQueue {
 public:
  ...
  using Priority = ::webrtc_mac_capturer::TaskQueueFactory::Priority;

  explicit TaskQueue(std::unique_ptr<webrtc_mac_capturer::TaskQueueBase,
                                     webrtc_mac_capturer::TaskQueueDeleter> task_queue);
  ~TaskQueue();
  ...
  webrtc_mac_capturer::TaskQueueBase* Get() { return impl_; }

  // TODO(tommi): For better debuggability, implement RTC_FROM_HERE.

  // Ownership of the task is passed to PostTask.
  void PostTask(std::unique_ptr<webrtc_mac_capturer::QueuedTask> task);
  ...
  void PostDelayedTask(std::unique_ptr<webrtc_mac_capturer::QueuedTask> task,
  ...
  template <class Closure,
            typename std::enable_if<!std::is_convertible<
                Closure,
                std::unique_ptr<webrtc_mac_capturer::QueuedTask>>::value>::type* = nullptr>
  void PostTask(Closure&& closure) {
    PostTask(webrtc_mac_capturer::ToQueuedTask(std::forward<Closure>(closure)));
  }
  ...
  void PostDelayedTask(Closure&& closure, uint32_t milliseconds) {
    PostDelayedTask(webrtc_mac_capturer::ToQueuedTask(std::forward<Closure>(closure)),
                    milliseconds);
  }

 private:
  webrtc_mac_capturer::TaskQueueBase* const impl_;
