...
void PhysicalSocketTest::ConnectInternalAcceptError(const IPAddress& loopback) {
  webrtc_mac_capturer::testing::StreamSink sink;
  ...
  EXPECT_FALSE(sink.Check(server.get(), webrtc_mac_capturer::testing::SSE_READ));
  ...
  EXPECT_FALSE(sink.Check(client1.get(), webrtc_mac_capturer::testing::SSE_OPEN));
  EXPECT_FALSE(sink.Check(client1.get(), webrtc_mac_capturer::testing::SSE_CLOSE));

  // Server has pending connection, try to accept it (will fail).
  EXPECT_TRUE_WAIT((sink.Check(server.get(), webrtc_mac_capturer::testing::SSE_READ)),
                   kTimeout);
  ...
  EXPECT_FALSE(sink.Check(client2.get(), webrtc_mac_capturer::testing::SSE_OPEN));
  EXPECT_FALSE(sink.Check(client2.get(), webrtc_mac_capturer::testing::SSE_CLOSE));
