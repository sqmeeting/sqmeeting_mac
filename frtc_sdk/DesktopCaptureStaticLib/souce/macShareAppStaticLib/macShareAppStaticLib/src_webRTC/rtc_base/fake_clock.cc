...
int64_t FakeClock::TimeNanos() const {
  webrtc_mac_capturer::MutexLock lock(&lock_);
  return time_ns_;
}

void FakeClock::SetTime(webrtc_mac_capturer::Timestamp new_time) {
  webrtc_mac_capturer::MutexLock lock(&lock_);
  RTC_DCHECK(new_time.us() * 1000 >= time_ns_);
  time_ns_ = new_time.us() * 1000;
}

void FakeClock::AdvanceTime(webrtc_mac_capturer::TimeDelta delta) {
  webrtc_mac_capturer::MutexLock lock(&lock_);
  time_ns_ += delta.ns();
}

void ThreadProcessingFakeClock::SetTime(webrtc_mac_capturer::Timestamp time) {
  clock_.SetTime(time);
...
void ThreadProcessingFakeClock::AdvanceTime(webrtc_mac_capturer::TimeDelta delta) {
