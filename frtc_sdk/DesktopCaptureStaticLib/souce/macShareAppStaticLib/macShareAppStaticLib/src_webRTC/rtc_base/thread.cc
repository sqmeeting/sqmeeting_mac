...
void Thread::SetDispatchWarningMs(int deadline) {
  if (!IsCurrent()) {
    PostTask(webrtc_mac_capturer::ToQueuedTask(
...
void Thread::QueuedTaskHandler::OnMessage(Message* msg) {
  RTC_DCHECK(msg);
  auto* data = static_cast<ScopedMessageData<webrtc_mac_capturer::QueuedTask>*>(msg->pdata);
  std::unique_ptr<webrtc_mac_capturer::QueuedTask> task(data->Release());
...
#if (!defined(NDEBUG) || RTC_DCHECK_IS_ON)
  if (!IsCurrent()) {
    PostTask(webrtc_mac_capturer::ToQueuedTask([this]() { DisallowAllInvokes(); }));
    return;
  }
  RTC_DCHECK_RUN_ON(this);
...
void Thread::PostTask(std::unique_ptr<webrtc_mac_capturer::QueuedTask> task) {
  ...
  Post(RTC_FROM_HERE, &queued_task_handler_,
       /*id=*/0, new ScopedMessageData<webrtc_mac_capturer::QueuedTask>(std::move(task)));
}

void Thread::PostDelayedTask(std::unique_ptr<webrtc_mac_capturer::QueuedTask> task,
  ...
  PostDelayed(RTC_FROM_HERE, milliseconds, &queued_task_handler_,
              /*id=*/0,
              new ScopedMessageData<webrtc_mac_capturer::QueuedTask>(std::move(task)));
}
