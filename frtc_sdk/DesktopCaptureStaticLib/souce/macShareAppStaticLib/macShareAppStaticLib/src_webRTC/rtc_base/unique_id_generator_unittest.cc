...
class FakeTaskQueue : public webrtc_mac_capturer::TaskQueueBase {
 public:
  FakeTaskQueue() : task_queue_setter_(this) {}

  void Delete() override {}
  void PostTask(std::unique_ptr<webrtc_mac_capturer::QueuedTask> task) override {}
...
TEST(UniqueNumberGenerator, UsedOnSecondaryThread) {
  const auto* current_tq = webrtc_mac_capturer::TaskQueueBase::Current();
  ...
  ASSERT_NE(current_tq, webrtc_mac_capturer::TaskQueueBase::Current());
