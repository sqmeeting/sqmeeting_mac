...
bool FileRotatingStream::OpenCurrentFile() {
  ...
  int error;
  file_ = webrtc_mac_capturer::FileWrapper::OpenWriteOnly(file_path, &error);
  if (!file_.is_open()) {
...
size_t FileRotatingStreamReader::ReadAll(void* buffer, size_t size) const {
  ...
  for (const auto& file_name : file_names_) {
    if (done < size) {
      webrtc_mac_capturer::FileWrapper f = webrtc_mac_capturer::FileWrapper::OpenReadOnly(file_name);
      if (!f.is_open()) {
        break;
      }
