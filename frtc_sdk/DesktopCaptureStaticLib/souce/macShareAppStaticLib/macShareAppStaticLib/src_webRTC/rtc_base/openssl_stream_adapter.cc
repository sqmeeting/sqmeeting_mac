...
bool ShouldAllowLegacyTLSProtocols() {
  return g_use_legacy_tls_protocols_override.load()
             ...
             : webrtc_mac_capturer::field_trial::IsEnabled("WebRTC-LegacyTlsProtocols");
}
...
void OpenSSLStreamAdapter::PostEvent(int events, int err) {
  owner_->PostTask(webrtc_mac_capturer::ToQueuedTask(
...
void OpenSSLStreamAdapter::SetTimeout(int delay_ms) {
  ...
  timeout_task_ = webrtc_mac_capturer::RepeatingTaskHandle::DelayedStart(
      owner_, webrtc_mac_capturer::TimeDelta::Millis(delay_ms),
      [flag = task_safety_.flag(), this]() {
        ...
        return webrtc_mac_capturer::TimeDelta::PlusInfinity();
      });
}
