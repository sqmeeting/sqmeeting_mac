...
struct AsyncResolver::State : public RefCountedBase {
  webrtc_mac_capturer::Mutex mutex;
  enum class Status {
    kLive,
    kDead
...
AsyncResolver::~AsyncResolver() {
  ...
  webrtc_mac_capturer::MutexLock lock(&state_->mutex);
...
void AsyncResolver::Start(const SocketAddress& addr) {
  ...
  PlatformThread::SpawnDetached(
      [this, addr, caller_task_queue = webrtc_mac_capturer::TaskQueueBase::Current(),
       state = state_] {
        std::vector<IPAddress> addresses;
        int error =
            ResolveHostname(addr.hostname().c_str(), addr.family(), &addresses);
        webrtc_mac_capturer::MutexLock lock(&state->mutex);
        if (state->status == State::Status::kLive) {
          caller_task_queue->PostTask(webrtc_mac_capturer::ToQueuedTask(
