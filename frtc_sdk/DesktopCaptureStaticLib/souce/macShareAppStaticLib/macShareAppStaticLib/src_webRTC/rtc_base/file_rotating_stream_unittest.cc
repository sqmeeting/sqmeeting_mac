...
void CleanupLogDirectory(const FileRotatingStream& stream) {
  for (size_t i = 0; i < stream.GetNumFiles(); ++i) {
    ...
    webrtc_mac_capturer::test::RemoveFile(stream.GetFilePath(i));
  }
}
...
class MAYBE_FileRotatingStreamTest : public ::testing::Test {
 protected:
  ...
  void Init(const std::string& dir_name,
    ...
    dir_path_ = webrtc_mac_capturer::test::OutputPath();

    // Append per-test output path in order to run within gtest parallel.
    dir_path_.append(dir_name);
    if (ensure_trailing_delimiter) {
      dir_path_.append(webrtc_mac_capturer::test::kPathDelimiter);
    }
    ASSERT_TRUE(webrtc_mac_capturer::test::CreateDir(dir_path_));
  ...
  void TearDown() override {
    ...
    EXPECT_TRUE(webrtc_mac_capturer::test::RemoveDir(dir_path_));
  ...
  void VerifyFileContents(const char* expected_contents,
    ...
    webrtc_mac_capturer::FileWrapper f = webrtc_mac_capturer::FileWrapper::OpenReadOnly(file_path);
...
TEST_F(MAYBE_FileRotatingStreamTest, EmptyWrite) {
  ...
  webrtc_mac_capturer::FileWrapper f = webrtc_mac_capturer::FileWrapper::OpenReadOnly(logfile_path);
  ASSERT_TRUE(f.is_open());
  char buf[1];
...
TEST_F(MAYBE_FileRotatingStreamTest, WriteAndRead) {
  ...
  for (size_t i = 0; i < arraysize(messages); ++i) {
    EXPECT_TRUE(webrtc_mac_capturer::test::FileExists(stream_->GetFilePath(i)));
  }
...
TEST_F(MAYBE_FileRotatingStreamTest, WriteWithoutDelimiterAndRead) {
  ...
  std::string expected_contents("bbccd");
  VerifyStreamRead(expected_contents.c_str(), expected_contents.size(),
                   dir_path_ + webrtc_mac_capturer::test::kPathDelimiter, kFilePrefix);
}
