...
constexpr bool is_sorted_and_unique(const Range& range, Comp comp) {
  ...
  return absl::c_adjacent_find(range, webrtc_mac_capturer::not_fn(comp)) == std::end(range);
}
...
class flat_tree {
 public:
  ...
  // the input be sorted.
  //
  // When passing the webrtc_mac_capturer::sorted_unique tag as the first argument no sort
  ...
  //
  // Prefer webrtc_mac_capturer::EraseIf() or some other variation on erase(remove(), end())
 ...
 private:
  ...
  void sort_and_unique(iterator first, iterator last) {
    ...
    auto equal_comp = webrtc_mac_capturer::not_fn(value_comp());
...
auto flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::find(const K& key)
    -> iterator {
  return const_cast_it(webrtc_mac_capturer::as_const(*this).find(key));
}
...
auto flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::equal_range(
  ...
  auto res = webrtc_mac_capturer::as_const(*this).equal_range(key);
...
auto flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::lower_bound(
  ...
  return const_cast_it(webrtc_mac_capturer::as_const(*this).lower_bound(key));
}
...
auto flat_tree<Key, GetKeyFromValue, KeyCompare, Container>::upper_bound(
    const K& key) -> iterator {
  return const_cast_it(webrtc_mac_capturer::as_const(*this).upper_bound(key));
}
...
template <class Key,
          ...
          typename Predicate>
size_t EraseIf(
    webrtc_mac_capturer::flat_containers_internal::
