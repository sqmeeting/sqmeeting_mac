...
absl::optional<ForcedFallbackParams> ParseFallbackParamsFromFieldTrials(
  ...
  const std::string field_trial =
      webrtc_mac_capturer::field_trial::FindFullName(kVp8ForceFallbackEncoderFieldTrial);
...
class VideoEncoderSoftwareFallbackWrapper final : public VideoEncoder {
 public:
  VideoEncoderSoftwareFallbackWrapper(
      std::unique_ptr<webrtc_mac_capturer::VideoEncoder> sw_encoder,
      std::unique_ptr<webrtc_mac_capturer::VideoEncoder> hw_encoder,
      bool prefer_temporal_support);
 ...
 private:
  ...
  const std::unique_ptr<webrtc_mac_capturer::VideoEncoder> encoder_;
  const std::unique_ptr<webrtc_mac_capturer::VideoEncoder> fallback_encoder_;
