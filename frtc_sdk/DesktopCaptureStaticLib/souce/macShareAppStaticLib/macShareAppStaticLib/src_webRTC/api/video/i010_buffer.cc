...
rtc::scoped_refptr<I010Buffer> I010Buffer::Rotate(
  ...
  if (rotation == webrtc_mac_capturer::kVideoRotation_0)
    return Copy(src);
  ...
  if (rotation == webrtc_mac_capturer::kVideoRotation_90 ||
      rotation == webrtc_mac_capturer::kVideoRotation_270) {
    std::swap(rotated_width, rotated_height);
  }

  rtc::scoped_refptr<webrtc_mac_capturer::I010Buffer> buffer =
  ...
  for (int x = 0; x < src.width(); x++) {
    for (int y = 0; y < src.height(); y++) {
      ...
      switch (rotation) {
        ...
        case webrtc_mac_capturer::kVideoRotation_0:
          RTC_NOTREACHED();
          break;
        case webrtc_mac_capturer::kVideoRotation_90:
          dest_x = src.height() - y - 1;
          dest_y = x;
          break;
        case webrtc_mac_capturer::kVideoRotation_180:
          dest_x = src.width() - x - 1;
          dest_y = src.height() - y - 1;
          break;
        case webrtc_mac_capturer::kVideoRotation_270:
          dest_x = y;
