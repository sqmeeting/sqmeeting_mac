...
struct RTC_EXPORT VideoSinkWants {
  ...
  // webrtc_mac_capturer::VideoFrame's resolution. In the case of encoding, we usually encode
  // at webrtc_mac_capturer::VideoFrame's resolution but this may not always be the case due
  ...
  // `max_pixel_count` constraints) create webrtc_mac_capturer::VideoFrames of size 480p, but
  // if we do scaleResolutionDownBy:2 then the only resolution we end up
  // encoding is 240p. In this case we still need to provide webrtc_mac_capturer::VideoFrames
  ...
  // should only be used as a hint when constructing the webrtc_mac_capturer::VideoFrame.
