...
class RTC_EXPORT EncodedImage {
 public:
  ...
  const webrtc_mac_capturer::ColorSpace* ColorSpace() const {
    return color_space_ ? &*color_space_ : nullptr;
  }
  void SetColorSpace(const absl::optional<webrtc_mac_capturer::ColorSpace>& color_space) {
 ...
 private:
  ...
  absl::optional<webrtc_mac_capturer::ColorSpace> color_space_;
  // This field is meant for media quality testing purpose only. When enabled it
  // carries the webrtc_mac_capturer::VideoFrame id field from the sender to the receiver.
