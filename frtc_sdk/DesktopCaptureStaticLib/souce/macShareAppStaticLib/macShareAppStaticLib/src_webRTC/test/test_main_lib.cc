...
class TestMainImpl : public TestMain {
 public:
  ...
  int Init() override {
    ...
    webrtc_mac_capturer::field_trial::InitFieldTrialsFromString(field_trials_.c_str());
  ...
  int Run(int argc, char* argv[]) override {
    ...
    if (!perf_output_file.empty()) {
      if (!webrtc_mac_capturer::test::WritePerfResults(perf_output_file)) {
        return 1;
      }
    }
    if (metrics_to_plot) {
      webrtc_mac_capturer::test::PrintPlottableResults(*metrics_to_plot);
    }
#endif
