...
uint16_t DefaultVideoQualityAnalyzer::OnFrameCaptured(
    ...
    const webrtc_mac_capturer::VideoFrame& frame) {
...
void DefaultVideoQualityAnalyzer::OnFrameEncoded(
    absl::string_view peer_name,
    uint16_t frame_id,
    const webrtc_mac_capturer::EncodedImage& encoded_image,
...
void DefaultVideoQualityAnalyzer::OnFrameDropped(
    absl::string_view peer_name,
    webrtc_mac_capturer::EncodedImageCallback::DropReason reason) {
...
void DefaultVideoQualityAnalyzer::OnFramePreDecode(
    absl::string_view peer_name,
    uint16_t frame_id,
    const webrtc_mac_capturer::EncodedImage& input_image) {
  MutexLock lock(&lock_);
  size_t peer_index = peers_->index(peer_name);
...
void DefaultVideoQualityAnalyzer::ReportResults() {
  using ::webrtc_mac_capturer::test::ImproveDirection;
