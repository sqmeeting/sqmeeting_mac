...
class DefaultVideoQualityAnalyzer : public VideoQualityAnalyzerInterface {
 public:
  explicit DefaultVideoQualityAnalyzer(
      webrtc_mac_capturer::Clock* clock,
 ...
 private:
  ...
  class FrameInFlight {
   public:
    ...
    void SetPreEncodeTime(webrtc_mac_capturer::Timestamp time) { pre_encode_time_ = time; }

    void OnFrameEncoded(webrtc_mac_capturer::Timestamp time,
    ...
    void OnFramePreDecode(size_t peer,
                          webrtc_mac_capturer::Timestamp received_time,
                          webrtc_mac_capturer::Timestamp decode_start_time);

    bool HasReceivedTime(size_t peer) const;

    void SetDecodeEndTime(size_t peer, webrtc_mac_capturer::Timestamp time) {
    ...
    void OnFrameRendered(size_t peer,
                         webrtc_mac_capturer::Timestamp time,
                         int width,
                         int height);
    ...
    webrtc_mac_capturer::Timestamp rendered_time(size_t peer) const {
      return receiver_stats_.at(peer).rendered_time;
    }

    void MarkDropped(size_t peer) { receiver_stats_[peer].dropped = true; }

    void SetPrevFrameRenderedTime(size_t peer, webrtc_mac_capturer::Timestamp time) {
  ...
  static void ReportResult(const std::string& metric_name,
                           ...
                           webrtc_mac_capturer::test::ImproveDirection improve_direction =
                               webrtc_mac_capturer::test::ImproveDirection::kNone);
  ...
  webrtc_mac_capturer::Clock* const clock_;
