...
void PeerConnectionE2EQualityTest::Run(RunParams run_params) {
  ...
  task_queue_ = std::make_unique<webrtc_mac_capturer::TaskQueueForTest>(
      time_controller_.GetTaskQueueFactory()->CreateTaskQueue(
          "pc_e2e_quality_test", webrtc_mac_capturer::TaskQueueFactory::Priority::NORMAL));
  ...
  if (alice_->params()->rtc_event_log_path) {
    auto alice_rtc_event_log = std::make_unique<webrtc_mac_capturer::RtcEventLogOutputFile>(
        alice_->params()->rtc_event_log_path.value());
    alice_->pc()->StartRtcEventLog(std::move(alice_rtc_event_log),
                                   webrtc_mac_capturer::RtcEventLog::kImmediateOutput);
  }
  if (bob_->params()->rtc_event_log_path) {
    auto bob_rtc_event_log = std::make_unique<webrtc_mac_capturer::RtcEventLogOutputFile>(
