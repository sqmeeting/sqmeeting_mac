...
class FileUtilsTest : public ::testing::Test {
 protected:
  ...
  static void SetUpTestSuite() {
    original_working_dir_ = webrtc_mac_capturer::test::WorkingDir();
  }
...
TEST_F(FileUtilsTest, OutputPathFromUnchangedWorkingDir) {
  ...
  std::string result = webrtc_mac_capturer::test::OutputPath();
...
TEST_F(FileUtilsTest, TempFilename) {
  std::string temp_filename = webrtc_mac_capturer::test::TempFilename(
      webrtc_mac_capturer::test::OutputPath(), "TempFilenameTest");
  ASSERT_TRUE(webrtc_mac_capturer::test::FileExists(temp_filename))
      << "Couldn't find file: " << temp_filename;
  remove(temp_filename.c_str());
}

TEST_F(FileUtilsTest, GenerateTempFilename) {
  std::string temp_filename = webrtc_mac_capturer::test::GenerateTempFilename(
      webrtc_mac_capturer::test::OutputPath(), "TempFilenameTest");
  ASSERT_FALSE(webrtc_mac_capturer::test::FileExists(temp_filename))
...
TEST_F(FileUtilsTest, MAYBE_CreateDir) {
  ...
  ASSERT_TRUE(webrtc_mac_capturer::test::CreateDir(directory));
  remove(directory.c_str());
}
...
TEST_F(FileUtilsTest, WorkingDirReturnsValue) {
  ...
  std::string working_dir = webrtc_mac_capturer::test::WorkingDir();
  ASSERT_GT(working_dir.length(), 0u);
}

TEST_F(FileUtilsTest, ResourcePathReturnsCorrectPath) {
  std::string result = webrtc_mac_capturer::test::ResourcePath(
...
TEST_F(FileUtilsTest, ResourcePathFromRootWorkingDir) {
  ASSERT_EQ(0, chdir(kPathDelimiter));
  std::string resource = webrtc_mac_capturer::test::ResourcePath("whatever", "ext");
#if !defined(WEBRTC_IOS)
...
TEST_F(FileUtilsTest, GetFileSizeExistingFile) {
  ...
  std::string temp_filename = webrtc_mac_capturer::test::TempFilename(
      webrtc_mac_capturer::test::OutputPath(), "fileutils_unittest");
  FILE* file = fopen(temp_filename.c_str(), "wb");
  ASSERT_TRUE(file != NULL) << "Failed to open file: " << temp_filename;
  ASSERT_GT(fprintf(file, "%s", "Dummy data"), 0)
      << "Failed to write to file: " << temp_filename;
  fclose(file);
  ASSERT_GT(webrtc_mac_capturer::test::GetFileSize(std::string(temp_filename.c_str())), 0u);
  remove(temp_filename.c_str());
}

TEST_F(FileUtilsTest, GetFileSizeNonExistingFile) {
  ASSERT_EQ(0u, webrtc_mac_capturer::test::GetFileSize("non-existing-file.tmp"));
}

TEST_F(FileUtilsTest, DirExists) {
  // Check that an existing directory is recognized as such.
  ASSERT_TRUE(webrtc_mac_capturer::test::DirExists(webrtc_mac_capturer::test::OutputPath()))
  ...
  ASSERT_FALSE(webrtc_mac_capturer::test::DirExists(directory))
  ...
  ASSERT_TRUE(webrtc_mac_capturer::test::FileExists(temp_filename))
      << "Couldn't find file: " << temp_filename;
  ASSERT_FALSE(webrtc_mac_capturer::test::DirExists(temp_filename))
