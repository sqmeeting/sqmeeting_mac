...
class VideoFrameWriter {
 public:
  ...
  virtual bool WriteFrame(const webrtc_mac_capturer::VideoFrame& frame) = 0;

  virtual void Close() = 0;
};

// Writes webrtc_mac_capturer::VideoFrame to specified file with y4m frame writer
class Y4mVideoFrameWriterImpl : public VideoFrameWriter {
 public:
  ...
  bool WriteFrame(const webrtc_mac_capturer::VideoFrame& frame) override;
  void Close() override;
...
// Writes webrtc_mac_capturer::VideoFrame to specified file with yuv frame writer
