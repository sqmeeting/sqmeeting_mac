...
std::unique_ptr<ScenarioIceConnection> ScenarioIceConnection::Create(
    webrtc_mac_capturer::test::NetworkEmulationManagerImpl* net,
    IceConnectionObserver* observer) {
...
void ScenarioIceConnectionImpl::SetRemoteSdp(SdpType type,
  ...
  remote_description_ = webrtc_mac_capturer::CreateSessionDescription(type, remote_sdp);
...
void ScenarioIceConnectionImpl::SetLocalSdp(SdpType type,
  ...
  RTC_DCHECK_RUN_ON(signaling_thread_);
  local_description_ = webrtc_mac_capturer::CreateSessionDescription(type, local_sdp);
