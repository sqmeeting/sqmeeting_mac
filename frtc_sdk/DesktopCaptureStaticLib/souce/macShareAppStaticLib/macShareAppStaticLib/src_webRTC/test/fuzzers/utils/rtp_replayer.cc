...
void RtpReplayer::Replay(
  ...
  // Work around: webrtc calls webrtc_mac_capturer::Random(clock.TimeInMicroseconds())
  // everywhere and Random expects non-zero seed. Let's set the clock non-zero
  // to make them happy.
  fake_clock.SetTime(webrtc_mac_capturer::Timestamp::Millis(1));
  ...
  webrtc_mac_capturer::RtcEventLogNull event_log;
...
void RtpReplayer::ReplayPackets(rtc::FakeClock* clock,
  ...
  while (true) {
    ...
    if (deliver_in_ms > 0) {
      ...
      clock->AdvanceTime(webrtc_mac_capturer::TimeDelta::Millis(
          std::min(deliver_in_ms, static_cast<int64_t>(100))));
    }

    rtc::CopyOnWriteBuffer packet_buffer(packet.data, packet.length);
    ++num_packets;
    switch (call->Receiver()->DeliverPacket(webrtc_mac_capturer::MediaType::VIDEO,
                                            packet_buffer,
                                            /* packet_time_us */ -1)) {
      case PacketReceiver::DELIVERY_OK:
        break;
      case PacketReceiver::DELIVERY_UNKNOWN_SSRC: {
        webrtc_mac_capturer::RtpPacket header;
