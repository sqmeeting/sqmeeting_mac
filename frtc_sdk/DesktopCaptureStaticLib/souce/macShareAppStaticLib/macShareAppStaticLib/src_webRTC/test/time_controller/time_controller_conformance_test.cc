...
std::string ParamsToString(const TestParamInfo<webrtc_mac_capturer::TimeMode>& param) {
  switch (param.param) {
    case webrtc_mac_capturer::TimeMode::kRealTime:
      return "RealTime";
    case webrtc_mac_capturer::TimeMode::kSimulated:
...
class SimulatedRealTimeControllerConformanceTest
    : public TestWithParam<webrtc_mac_capturer::TimeMode> {};
...
TEST_P(SimulatedRealTimeControllerConformanceTest,
  ...
  auto task_queue = time_controller->GetTaskQueueFactory()->CreateTaskQueue(
      "task_queue", webrtc_mac_capturer::TaskQueueFactory::Priority::NORMAL);
