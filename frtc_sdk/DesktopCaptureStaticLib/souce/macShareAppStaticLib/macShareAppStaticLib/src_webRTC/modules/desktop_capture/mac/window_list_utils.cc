...
static_assert(static_cast<webrtc_mac_capturer::WindowId>(kCGNullWindowID) ==
                  webrtc_mac_capturer::kNullWindowId,
...
bool GetWindowList(rtc::FunctionView<bool(CFDictionaryRef)> on_window,
  ...
  // decreasing z-order.
    CFArrayRef window_array = CGWindowListCopyWindowInfo(kCGWindowListOptionOnScreenOnly | kCGWindowListExcludeDesktopElements,
                                                         kCGNullWindowID);
    if (!window_array)
        return false;
  ...
  for (CFIndex i = 0; i < count; i++) {
    ...
    if (ignore_minimized &&
        !IsWindowOnScreen(window) &&
        !IsWindowFullScreen(desktop_config, window)) {
      continue;
    }

    // If window title is empty, only consider it if it is either on screen or fullscreen.
  ...
  const bool ret = GetWindowList(
      [&sources, &pid_itr_map](CFDictionaryRef window) {
        ...
        if (window_id != kNullWindowId) {
          ...
          std::map<int, std::list<DesktopCapturer::Source>::const_iterator>::iterator itr = pid_itr_map.find(pid);
          ...
          if (title.empty()) {
            ...
            if (!owner_name.empty() && (itr == pid_itr_map.end())) {
              ...
              std::list<DesktopCapturer::Source>::const_iterator last_source = --sources.end();
              pid_itr_map.insert(std::pair<int, std::list<DesktopCapturer::Source>::const_iterator>(pid, last_source));
            }
          } else {
            ...
            if (itr != pid_itr_map.end() && (itr->second != sources.end())) {
              ...
              // sdt::list::end() never changes during the lifetime of that list.
...
bool IsWindowFullScreen(const MacDesktopConfiguration& desktop_config,
  ...
  if (bounds_ref &&
    ...
    //printf("[%s][%d]: bounds.size: [%d, %d, %d, %d] \n", __func__, __LINE__, (int)bounds.origin.x, (int)bounds.origin.y, (int)bounds.size.width, (int)bounds.size.height);
    //printf("[%s][%d]: bounds.size: MakeXYWH[x, y, x+w, y+h]: [%d, %d, %d, %d] \n", __func__, __LINE__, (int)bounds.origin.x, (int)bounds.origin.y, (int)bounds.origin.x+(int)bounds.size.width, (int)bounds.origin.y+(int)bounds.size.height);
    ...
    for (MacDisplayConfigurations::const_iterator it =
      ...
      if (it->bounds.equals(
        ...
        fullscreen = true;
        //printf("[%s][%d]: windowID : %d, fullscreen: %s, ->break \n", __func__, __LINE__, (int)windowID, fullscreen?"true":"false");
        break;
      }
    }
  }
  ...
  //printf("[%s][%d]: windowID : %d, fullscreen: %s \n", __func__, __LINE__, (int)windowID, fullscreen?"true":"false");
  return fullscreen;
}
...
bool IsWindowFullScreen(const MacDesktopConfiguration& desktop_config,
  ...
  GetWindowRef(id, [&](CFDictionaryRef window) {
    ...
    //printf("[%s][%d]: window id: %d, fullscreen: %s \n", __func__, __LINE__, (int)id, fullscreen?"true":"false");
  });
  return fullscreen;
}

//[Note]: For WPS support PPT slide mode, fullscreen window, the y is larger (+34) than the fullscreen's y value.
// Returns true if the window is occupying a full screen.
bool IsWPSWindowFullScreen(const MacDesktopConfiguration& desktop_config,
                           CFDictionaryRef window) {
    
  CFNumberRef window_id = reinterpret_cast<CFNumberRef>(CFDictionaryGetValue(window, kCGWindowNumber));
  int windowID = 0;
  CFNumberGetValue((CFNumberRef)window_id, kCFNumberIntType, &windowID);
  ...
  if (bounds_ref &&
    ...
    //printf("[%s][%d]: windowID: %d, bounds.size: [%d, %d, %d, %d] \n", __func__, __LINE__, (int)windowID, (int)bounds.origin.x, (int)bounds.origin.y, (int)bounds.size.width, (int)bounds.size.height);
    //printf("[%s][%d]: bounds.size: WPSMakeXYWH[x, y-34, x+w, y+h]: [%d, %d, %d, %d] \n", __func__, __LINE__, (int)bounds.origin.x, (int)bounds.origin.y-34, (int)bounds.origin.x+(int)bounds.size.width, (int)bounds.origin.y+(int)bounds.size.height);
    ...
    for (MacDisplayConfigurations::const_iterator it =
      ...
      //printf("[%s][%d]: desktop_config.display[%d], bounds.size: [%d, %d, %d, %d] \n", __func__, __LINE__, index_display++, (int)it->bounds.left(), (int)it->bounds.top(), (int)it->bounds.width(), (int)it->bounds.height());

      //1.WPS slide show on MacBook build-in Display's Screen.
      if (it->bounds.equals(
              DesktopRect::WPSMakeXYWH(bounds.origin.x, bounds.origin.y,
                                       bounds.size.width, bounds.size.height))) {
        fullscreen = true;
        //printf("[%s][%d]: WPSMakeXYWH(), windowID : %d, -> set fullscreen = true ->break \n", __func__, __LINE__, (int)windowID);
        break;
      }
      
      //2.WPS slide show on extend Display's screen, not on MacBook build-in Display.
      if (it->bounds.equals(
             DesktopRect::WPSMakeXYWHVer2(bounds.origin.x, bounds.origin.y,
                                          bounds.size.width, bounds.size.height))) {
        fullscreen = true;
        //printf("[%s][%d]: WPSMakeXYWHVer2(), windowID : %d, -> set fullscreen = true ->break \n", __func__, __LINE__, (int)windowID);
        break;
      }
...
bool IsWPSWindowFullScreen(const MacDesktopConfiguration& desktop_config,
  ...
  bool fullscreen = false;
  GetWindowRef(id, [&](CFDictionaryRef window) {
    fullscreen = IsWPSWindowFullScreen(desktop_config, window);
    //printf("[%s][%d]: window id: %d, fullscreen: %s \n", __func__, __LINE__, (int)id, fullscreen?"true":"false");
  });
  return fullscreen;
}
