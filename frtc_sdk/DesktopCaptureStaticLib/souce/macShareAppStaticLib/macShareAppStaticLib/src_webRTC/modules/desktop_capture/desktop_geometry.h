...
class RTC_EXPORT DesktopRect {
 public:
  ...
  //[Note]: for WPS support PPT. -yingyong.Mao -2023-8-28
  /*
   1.when the WPS full screen window on macbook's build-in display,
     the y will be different with different resolution of the display;
     but the y+height will equal to the height of the macbook's build-in display.
   
   such as:
   set the macBook build-in display to 1352 * 878, then the WPS fullscreen window height is 844, the title bar is 34.
   set the macBook build-in display to 1512 * 982, then the WPS fullscreen window height is 945, the title bar is 37.
   set the macBook build-in display to 1800 * 1169, then the WPS fullscreen window height is 1126, the title bar is 43.
  */
  static DesktopRect WPSMakeXYWH(int32_t x,
                                 int32_t y,
                                 int32_t width,
                                 int32_t height) {
    return DesktopRect(x, 0, x + width, y + height);
  }

 /*
  2.when the WPS full screen window on expand display's screen,
   the y will be the same with different resolution of the display.
 */
  static DesktopRect WPSMakeXYWHVer2(int32_t x,
                                     int32_t y,
                                     int32_t width,
                                     int32_t height) {
    return DesktopRect(x, y, x + width, y + height);
