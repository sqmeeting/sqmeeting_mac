...
bool WindowCapturerMac::SelectSource(SourceId id) {
    ...
    CGWindowID convertedWindowID = (CGWindowID)(uintptr_t)id;
  //if (!IsWindowValid(id))
  if (!IsWindowValid(convertedWindowID)) {
      return false;
  }
  //window_id_ = id;
    window_id_ = convertedWindowID;
    return true;
}
  ...
  if (full_screen_window_detector_) {
    full_screen_window_detector_->UpdateWindowListIfNeeded(
        window_id_, [](DesktopCapturer::SourceList* sources) {
          ...
          return webrtc_mac_capturer::GetWindowList(
    ...
    DesktopCapturer::SourceId sourceID = full_screen_window_detector_->FindFullScreenWindow(window_id_);
    CGWindowID full_screen_window = (CGWindowID)(uintptr_t)sourceID;
