...
static jlong JNI_LibaomAv1Encoder_CreateEncoder(JNIEnv* jni) {
  return jlongFromPointer(webrtc_mac_capturer::CreateLibaomAv1Encoder().release());
}

static jboolean JNI_LibaomAv1Encoder_IsSupported(JNIEnv* jni) {
  return webrtc_mac_capturer::kIsLibaomAv1EncoderSupported;
}

static jlong JNI_LibaomAv1Decoder_CreateDecoder(JNIEnv* jni) {
  return jlongFromPointer(webrtc_mac_capturer::CreateLibaomAv1Decoder().release());
}

static jboolean JNI_LibaomAv1Decoder_IsSupported(JNIEnv* jni) {
  return webrtc_mac_capturer::kIsLibaomAv1DecoderSupported;
}
