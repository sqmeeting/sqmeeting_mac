...
webrtc_mac_capturer::PortPrunePolicy JavaToNativePortPrunePolicy(
    JNIEnv* jni,
  ...
  if (enum_name == "NO_PRUNE") {
    return webrtc_mac_capturer::NO_PRUNE;
  }
  if (enum_name == "PRUNE_BASED_ON_PRIORITY") {
    return webrtc_mac_capturer::PRUNE_BASED_ON_PRIORITY;
  }
  if (enum_name == "KEEP_FIRST_READY") {
    return webrtc_mac_capturer::KEEP_FIRST_READY;
  }
