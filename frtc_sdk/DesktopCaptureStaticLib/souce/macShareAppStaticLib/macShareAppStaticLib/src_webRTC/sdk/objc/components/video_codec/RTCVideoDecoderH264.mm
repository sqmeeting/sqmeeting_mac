...
- (NSInteger)decode:(RTC_OBJC_TYPE(RTCEncodedImage) *)inputImage
  ...
  rtc::ScopedCFTypeRef<CMVideoFormatDescriptionRef> inputFormat =
      rtc::ScopedCF(webrtc_mac_capturer::CreateVideoFormatDescription((uint8_t *)inputImage.buffer.bytes,
  ...
  if (!webrtc_mac_capturer::H264AnnexBBufferToCMSampleBuffer((uint8_t *)inputImage.buffer.bytes,
                                                inputImage.buffer.length,
                                                _videoFormat,
                                                &sampleBuffer,
                                                _memoryPool)) {
