...
NSString * const kRTCMediaStreamTrackKindAudio =
    @(webrtc_mac_capturer::MediaStreamTrackInterface::kAudioKind);
NSString * const kRTCMediaStreamTrackKindVideo =
    @(webrtc_mac_capturer::MediaStreamTrackInterface::kVideoKind);

@implementation RTC_OBJC_TYPE (RTCMediaStreamTrack) {
  RTC_OBJC_TYPE(RTCPeerConnectionFactory) * _factory;
  rtc::scoped_refptr<webrtc_mac_capturer::MediaStreamTrackInterface> _nativeTrack;
...
- (rtc::scoped_refptr<webrtc_mac_capturer::MediaStreamTrackInterface>)nativeTrack {
  return _nativeTrack;
}
...
- (instancetype)initWithFactory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory
                    nativeTrack:(rtc::scoped_refptr<webrtc_mac_capturer::MediaStreamTrackInterface>)nativeTrack
...
- (instancetype)initWithFactory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory
  ...
  NSParameterAssert(nativeTrack);
  if (nativeTrack->kind() ==
      std::string(webrtc_mac_capturer::MediaStreamTrackInterface::kAudioKind)) {
    return [self initWithFactory:factory nativeTrack:nativeTrack type:RTCMediaStreamTrackTypeAudio];
  }
  if (nativeTrack->kind() ==
      std::string(webrtc_mac_capturer::MediaStreamTrackInterface::kVideoKind)) {
...
+ (webrtc_mac_capturer::MediaStreamTrackInterface::TrackState)nativeTrackStateForState:
    (RTCMediaStreamTrackState)state {
  switch (state) {
    case RTCMediaStreamTrackStateLive:
      return webrtc_mac_capturer::MediaStreamTrackInterface::kLive;
    case RTCMediaStreamTrackStateEnded:
      return webrtc_mac_capturer::MediaStreamTrackInterface::kEnded;
  }
}

+ (RTCMediaStreamTrackState)trackStateForNativeState:
    (webrtc_mac_capturer::MediaStreamTrackInterface::TrackState)nativeState {
  switch (nativeState) {
    case webrtc_mac_capturer::MediaStreamTrackInterface::kLive:
      return RTCMediaStreamTrackStateLive;
    case webrtc_mac_capturer::MediaStreamTrackInterface::kEnded:
...
+ (RTC_OBJC_TYPE(RTCMediaStreamTrack) *)
    mediaTrackForNativeTrack:(rtc::scoped_refptr<webrtc_mac_capturer::MediaStreamTrackInterface>)nativeTrack
                     factory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory {
  NSParameterAssert(nativeTrack);
  NSParameterAssert(factory);
  if (nativeTrack->kind() == webrtc_mac_capturer::MediaStreamTrackInterface::kAudioKind) {
    return [[RTC_OBJC_TYPE(RTCAudioTrack) alloc] initWithFactory:factory
                                                     nativeTrack:nativeTrack
                                                            type:RTCMediaStreamTrackTypeAudio];
  } else if (nativeTrack->kind() == webrtc_mac_capturer::MediaStreamTrackInterface::kVideoKind) {
