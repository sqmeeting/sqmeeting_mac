...
@implementation RTC_OBJC_TYPE (RTCDataBuffer) {
  std::unique_ptr<webrtc_mac_capturer::DataBuffer> _dataBuffer;
}
...
- (instancetype)initWithData:(NSData *)data isBinary:(BOOL)isBinary {
  ...
  if (self = [super init]) {
    ...
    _dataBuffer.reset(new webrtc_mac_capturer::DataBuffer(buffer, isBinary));
  }
  return self;
}
...
- (instancetype)initWithNativeBuffer:(const webrtc_mac_capturer::DataBuffer&)nativeBuffer {
  if (self = [super init]) {
    _dataBuffer.reset(new webrtc_mac_capturer::DataBuffer(nativeBuffer));
  }
  return self;
}

- (const webrtc_mac_capturer::DataBuffer *)nativeDataBuffer {
...
@implementation RTC_OBJC_TYPE (RTCDataChannel) {
  ...
  rtc::scoped_refptr<webrtc_mac_capturer::DataChannelInterface> _nativeDataChannel;
  std::unique_ptr<webrtc_mac_capturer::DataChannelDelegateAdapter> _observer;
...
- (instancetype)initWithFactory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory
              nativeDataChannel:
                  (rtc::scoped_refptr<webrtc_mac_capturer::DataChannelInterface>)nativeDataChannel {
  NSParameterAssert(nativeDataChannel);
  if (self = [super init]) {
    _factory = factory;
    _nativeDataChannel = nativeDataChannel;
    _observer.reset(new webrtc_mac_capturer::DataChannelDelegateAdapter(self));
    _nativeDataChannel->RegisterObserver(_observer.get());
  }
  return self;
}

+ (webrtc_mac_capturer::DataChannelInterface::DataState)
    nativeDataChannelStateForState:(RTCDataChannelState)state {
  switch (state) {
    case RTCDataChannelStateConnecting:
      return webrtc_mac_capturer::DataChannelInterface::DataState::kConnecting;
    case RTCDataChannelStateOpen:
      return webrtc_mac_capturer::DataChannelInterface::DataState::kOpen;
    case RTCDataChannelStateClosing:
      return webrtc_mac_capturer::DataChannelInterface::DataState::kClosing;
    case RTCDataChannelStateClosed:
      return webrtc_mac_capturer::DataChannelInterface::DataState::kClosed;
  }
}
...
+ (RTCDataChannelState)dataChannelStateForNativeState:
  ...
  switch (nativeState) {
    case webrtc_mac_capturer::DataChannelInterface::DataState::kConnecting:
      return RTCDataChannelStateConnecting;
    case webrtc_mac_capturer::DataChannelInterface::DataState::kOpen:
      return RTCDataChannelStateOpen;
    case webrtc_mac_capturer::DataChannelInterface::DataState::kClosing:
      return RTCDataChannelStateClosing;
    case webrtc_mac_capturer::DataChannelInterface::DataState::kClosed:
