...
- (void)testCreateVideoFormatDescription {
  CMVideoFormatDescriptionRef description =
      webrtc_mac_capturer::CreateVideoFormatDescription(SPS_PPS_BUFFER, arraysize(SPS_PPS_BUFFER));
  ...
  // clang-format on
  description = webrtc_mac_capturer::CreateVideoFormatDescription(sps_pps_not_at_start_buffer,
  ...
  XCTAssertFalse(webrtc_mac_capturer::CreateVideoFormatDescription(other_buffer, arraysize(other_buffer)));
}

- (void)testReadEmptyInput {
  const uint8_t annex_b_test_data[] = {0x00};
  webrtc_mac_capturer::AnnexBBufferReader reader(annex_b_test_data, 0);
...
- (void)testReadSingleNalu {
  ...
  webrtc_mac_capturer::AnnexBBufferReader reader(annex_b_test_data, arraysize(annex_b_test_data));
  const uint8_t* nalu = nullptr;
  size_t nalu_length = 0;
...
- (void)testEmptyOutputBuffer {
  ...
  webrtc_mac_capturer::AvccBufferWriter writer(buffer.get(), 0);
...
- (void)testWriteSingleNalu {
  ...
  std::unique_ptr<uint8_t[]> buffer(new uint8_t[buffer_size]);
  webrtc_mac_capturer::AvccBufferWriter writer(buffer.get(), buffer_size);
...
- (void)testH264AnnexBBufferToCMSampleBuffer {
  ...
  Boolean result = webrtc_mac_capturer::H264AnnexBBufferToCMSampleBuffer(annex_b_test_data,
...
- (void)testH264CMSampleBufferToAnnexBBuffer {
  ...
  Boolean result = webrtc_mac_capturer::H264CMSampleBufferToAnnexBBuffer(sample_buffer,
                                                            /* is_keyframe */ false,
                                                            &annexb_buffer);
