...
@interface RTCAudioDeviceTests : XCTestCase {
  ...
  std::unique_ptr<webrtc_mac_capturer::ios_adm::AudioDeviceIOS> _audio_device;
}
...
- (void)setUp {
  ...
  _audioDeviceModule = webrtc_mac_capturer::CreateAudioDeviceModule();
  _audio_device.reset(new webrtc_mac_capturer::ios_adm::AudioDeviceIOS(/*bypass_voice_processing=*/false));
...
- (void)testInterruptedAudioSession {
  ...
  std::unique_ptr<webrtc_mac_capturer::TaskQueueFactory> task_queue_factory =
      webrtc_mac_capturer::CreateDefaultTaskQueueFactory();
  std::unique_ptr<webrtc_mac_capturer::AudioDeviceBuffer> audio_buffer;
  audio_buffer.reset(new webrtc_mac_capturer::AudioDeviceBuffer(task_queue_factory.get()));
  _audio_device->AttachAudioBuffer(audio_buffer.get());
  XCTAssertEqual(webrtc_mac_capturer::AudioDeviceGeneric::InitStatus::OK, _audio_device->Init());
