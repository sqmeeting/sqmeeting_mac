...
class ObjCEncodedImageBuffer : public webrtc_mac_capturer::EncodedImageBufferInterface {
 public:
...
// A simple wrapper around webrtc_mac_capturer::EncodedImageBufferInterface to make it usable with associated
// objects.
@interface RTCWrappedEncodedImageBuffer : NSObject
@property(nonatomic) rtc::scoped_refptr<webrtc_mac_capturer::EncodedImageBufferInterface> buffer;
- (instancetype)initWithEncodedImageBuffer:
    (rtc::scoped_refptr<webrtc_mac_capturer::EncodedImageBufferInterface>)buffer;
@end
...
(Private)
    ...
    - (rtc::scoped_refptr<webrtc_mac_capturer::EncodedImageBufferInterface>)encodedData {
  RTCWrappedEncodedImageBuffer *wrappedBuffer =
      objc_getAssociatedObject(self, @selector(encodedData));
  return wrappedBuffer.buffer;
}

- (void)setEncodedData:(rtc::scoped_refptr<webrtc_mac_capturer::EncodedImageBufferInterface>)buffer {
...
- (instancetype)initWithNativeEncodedImage:(const webrtc_mac_capturer::EncodedImage &)encodedImage {
  if (self = [super init]) {
    ...
    self.contentType = (encodedImage.content_type_ == webrtc_mac_capturer::VideoContentType::SCREENSHARE) ?
...
- (webrtc_mac_capturer::EncodedImage)nativeEncodedImage {
  // Return the pointer without copying.
  webrtc_mac_capturer::EncodedImage encodedImage;
  if (self.encodedData) {
  ...
  encodedImage._frameType = webrtc_mac_capturer::VideoFrameType(self.frameType);
  encodedImage.rotation_ = webrtc_mac_capturer::VideoRotation(self.rotation);
  encodedImage.qp_ = self.qp ? self.qp.intValue : -1;
  encodedImage.content_type_ = (self.contentType == RTCVideoContentTypeScreenshare) ?
      webrtc_mac_capturer::VideoContentType::SCREENSHARE :
      webrtc_mac_capturer::VideoContentType::UNSPECIFIED;
