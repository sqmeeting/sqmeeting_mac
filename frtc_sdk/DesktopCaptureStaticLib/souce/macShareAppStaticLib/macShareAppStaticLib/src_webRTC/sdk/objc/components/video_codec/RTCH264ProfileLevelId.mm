...
NSString *MaxSupportedLevelForProfile(webrtc_mac_capturer::H264Profile profile) {
  const absl::optional<webrtc_mac_capturer::H264ProfileLevelId> profileLevelId =
  ...
  if (profileLevelId && profileLevelId->profile >= profile) {
    const absl::optional<std::string> profileString =
        H264ProfileLevelIdToString(webrtc_mac_capturer::H264ProfileLevelId(profile, profileLevelId->level));
    if (profileString) {
...
NSString *MaxSupportedProfileLevelConstrainedBaseline() {
#if defined(WEBRTC_IOS)
  NSString *profile = MaxSupportedLevelForProfile(webrtc_mac_capturer::H264Profile::kProfileConstrainedBaseline);
  if (profile != nil) {
    return profile;
  }
#endif
...
NSString *MaxSupportedProfileLevelConstrainedHigh() {
#if defined(WEBRTC_IOS)
  NSString *profile = MaxSupportedLevelForProfile(webrtc_mac_capturer::H264Profile::kProfileConstrainedHigh);
  if (profile != nil) {
    return profile;
  }
#endif
...
- (instancetype)initWithHexString:(NSString *)hexString {
  if (self = [super init]) {
    ...
    absl::optional<webrtc_mac_capturer::H264ProfileLevelId> profile_level_id =
        webrtc_mac_capturer::ParseH264ProfileLevelId([hexString cStringUsingEncoding:NSUTF8StringEncoding]);
...
- (instancetype)initWithProfile:(RTCH264Profile)profile level:(RTCH264Level)level {
  if (self = [super init]) {
    ...
    absl::optional<std::string> hex_string =
        webrtc_mac_capturer::H264ProfileLevelIdToString(webrtc_mac_capturer::H264ProfileLevelId(
            static_cast<webrtc_mac_capturer::H264Profile>(profile), static_cast<webrtc_mac_capturer::H264Level>(level)));
    self.hexString =
