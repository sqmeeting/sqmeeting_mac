...
- (void)testInitFromNativeSessionDescription {
  webrtc_mac_capturer::SessionDescriptionInterface *nativeDescription;

  nativeDescription = webrtc_mac_capturer::CreateSessionDescription(
      webrtc_mac_capturer::SessionDescriptionInterface::kAnswer,
      [self sdp].stdString,
      nullptr);

  RTC_OBJC_TYPE(RTCSessionDescription) *description =
      [[RTC_OBJC_TYPE(RTCSessionDescription) alloc] initWithNativeDescription:nativeDescription];
  EXPECT_EQ(webrtc_mac_capturer::SessionDescriptionInterface::kAnswer,
