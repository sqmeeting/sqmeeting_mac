...
@implementation RTC_OBJC_TYPE (RTCRtpSender) {
  ...
  rtc::scoped_refptr<webrtc_mac_capturer::RtpSenderInterface> _nativeRtpSender;
}
...
- (void)setFrameEncryptor:(rtc::scoped_refptr<webrtc_mac_capturer::FrameEncryptorInterface>)frameEncryptor {
  _nativeRtpSender->SetFrameEncryptor(frameEncryptor);
}

#pragma mark - Private

- (rtc::scoped_refptr<webrtc_mac_capturer::RtpSenderInterface>)nativeRtpSender {
  return _nativeRtpSender;
}

- (instancetype)initWithFactory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory
                nativeRtpSender:(rtc::scoped_refptr<webrtc_mac_capturer::RtpSenderInterface>)nativeRtpSender {
  NSParameterAssert(factory);
  NSParameterAssert(nativeRtpSender);
  if (self = [super init]) {
    _factory = factory;
    _nativeRtpSender = nativeRtpSender;
    rtc::scoped_refptr<webrtc_mac_capturer::DtmfSenderInterface> nativeDtmfSender(
