...
- (rtc::scoped_refptr<webrtc_mac_capturer::AudioDeviceModule>)audioDeviceModule {
#if defined(WEBRTC_IOS)
  return webrtc_mac_capturer::CreateAudioDeviceModule();
#else
  return nullptr;
#endif
}
...
#else
  return [self
      initWithNativeAudioEncoderFactory:webrtc_mac_capturer::CreateBuiltinAudioEncoderFactory()
              nativeAudioDecoderFactory:webrtc_mac_capturer::CreateBuiltinAudioDecoderFactory()
              nativeVideoEncoderFactory:webrtc_mac_capturer::ObjCToNativeVideoEncoderFactory([[RTC_OBJC_TYPE(
                                            RTCVideoEncoderFactoryH264) alloc] init])
              nativeVideoDecoderFactory:webrtc_mac_capturer::ObjCToNativeVideoDecoderFactory([[RTC_OBJC_TYPE(
...
- (instancetype)
  ...
  return [self initWithNoMedia];
#else
  std::unique_ptr<webrtc_mac_capturer::VideoEncoderFactory> native_encoder_factory;
  std::unique_ptr<webrtc_mac_capturer::VideoDecoderFactory> native_decoder_factory;
  if (encoderFactory) {
    native_encoder_factory = webrtc_mac_capturer::ObjCToNativeVideoEncoderFactory(encoderFactory);
  }
  if (decoderFactory) {
    native_decoder_factory = webrtc_mac_capturer::ObjCToNativeVideoDecoderFactory(decoderFactory);
  }
  return [self initWithNativeAudioEncoderFactory:webrtc_mac_capturer::CreateBuiltinAudioEncoderFactory()
                       nativeAudioDecoderFactory:webrtc_mac_capturer::CreateBuiltinAudioDecoderFactory()
...
- (instancetype)initWithNoMedia {
  if (self = [self initNative]) {
    webrtc_mac_capturer::PeerConnectionFactoryDependencies dependencies;
    dependencies.network_thread = _networkThread.get();
    dependencies.worker_thread = _workerThread.get();
    dependencies.signaling_thread = _signalingThread.get();
    if (webrtc_mac_capturer::field_trial::IsEnabled("WebRTC-Network-UseNWPathMonitor")) {
      dependencies.network_monitor_factory = webrtc_mac_capturer::CreateNetworkMonitorFactory();
    }
    _nativeFactory = webrtc_mac_capturer::CreateModularPeerConnectionFactory(std::move(dependencies));
...
- (instancetype)initWithNativeAudioEncoderFactory:
                    (rtc::scoped_refptr<webrtc_mac_capturer::AudioEncoderFactory>)audioEncoderFactory
                        ...
                        nativeVideoEncoderFactory:
                            (std::unique_ptr<webrtc_mac_capturer::VideoEncoderFactory>)videoEncoderFactory
                        nativeVideoDecoderFactory:
                            (std::unique_ptr<webrtc_mac_capturer::VideoDecoderFactory>)videoDecoderFactory
                                audioDeviceModule:(webrtc_mac_capturer::AudioDeviceModule *)audioDeviceModule
                    ...
                    (rtc::scoped_refptr<webrtc_mac_capturer::AudioEncoderFactory>)audioEncoderFactory
                        ...
                        nativeVideoDecoderFactory:
                         ...
                         networkControllerFactory:
                             (std::unique_ptr<webrtc_mac_capturer::NetworkControllerFactoryInterface>)
                                 networkControllerFactory {
  if (self = [self initNative]) {
    webrtc_mac_capturer::PeerConnectionFactoryDependencies dependencies;
    dependencies.network_thread = _networkThread.get();
...
#ifndef HAVE_NO_MEDIA
    dependencies.task_queue_factory = webrtc_mac_capturer::CreateDefaultTaskQueueFactory();
    dependencies.trials = std::make_unique<webrtc_mac_capturer::FieldTrialBasedConfig>();
    ...
    if (audioProcessingModule) {
      ...
      media_deps.audio_processing = webrtc_mac_capturer::AudioProcessingBuilder().Create();
    }
    media_deps.trials = dependencies.trials.get();
    dependencies.media_engine = cricket::CreateMediaEngine(std::move(media_deps));
    dependencies.call_factory = webrtc_mac_capturer::CreateCallFactory();
    dependencies.event_log_factory =
        std::make_unique<webrtc_mac_capturer::RtcEventLogFactory>(dependencies.task_queue_factory.get());
...
- (RTC_OBJC_TYPE(RTCAudioSource) *)audioSourceWithConstraints:
  ...
  rtc::scoped_refptr<webrtc_mac_capturer::AudioSourceInterface> source =
