...
- (instancetype)initWithNativeConfiguration:
    (const webrtc_mac_capturer::PeerConnectionInterface::RTCConfiguration &)config {
  if (self = [super init]) {
    _enableDscp = config.dscp();
    NSMutableArray *iceServers = [NSMutableArray array];
    for (const webrtc_mac_capturer::PeerConnectionInterface::IceServer& server : config.servers) {
    ...
    webrtc_mac_capturer::PeerConnectionInterface::ContinualGatheringPolicy nativePolicy =
...
- (webrtc_mac_capturer::PeerConnectionInterface::RTCConfiguration *)
    createNativeConfiguration {
  std::unique_ptr<webrtc_mac_capturer::PeerConnectionInterface::RTCConfiguration>
      nativeConfig(new webrtc_mac_capturer::PeerConnectionInterface::RTCConfiguration(
          webrtc_mac_capturer::PeerConnectionInterface::RTCConfigurationType::kAggressive));
  ...
  if (_cryptoOptions) {
    webrtc_mac_capturer::CryptoOptions nativeCryptoOptions;
    ...
    nativeConfig->crypto_options = absl::optional<webrtc_mac_capturer::CryptoOptions>(nativeCryptoOptions);
  }
...
+ (webrtc_mac_capturer::PeerConnectionInterface::IceTransportsType)
    nativeTransportsTypeForTransportPolicy:(RTCIceTransportPolicy)policy {
  switch (policy) {
    case RTCIceTransportPolicyNone:
      return webrtc_mac_capturer::PeerConnectionInterface::kNone;
    case RTCIceTransportPolicyRelay:
      return webrtc_mac_capturer::PeerConnectionInterface::kRelay;
    case RTCIceTransportPolicyNoHost:
      return webrtc_mac_capturer::PeerConnectionInterface::kNoHost;
    case RTCIceTransportPolicyAll:
      return webrtc_mac_capturer::PeerConnectionInterface::kAll;
  }
}
...
+ (RTCIceTransportPolicy)transportPolicyForTransportsType:
  ...
  switch (nativeType) {
    case webrtc_mac_capturer::PeerConnectionInterface::kNone:
      return RTCIceTransportPolicyNone;
    case webrtc_mac_capturer::PeerConnectionInterface::kRelay:
      return RTCIceTransportPolicyRelay;
    case webrtc_mac_capturer::PeerConnectionInterface::kNoHost:
      return RTCIceTransportPolicyNoHost;
    case webrtc_mac_capturer::PeerConnectionInterface::kAll:
...
+ (webrtc_mac_capturer::PeerConnectionInterface::BundlePolicy)nativeBundlePolicyForPolicy:
  ...
  switch (policy) {
    case RTCBundlePolicyBalanced:
      return webrtc_mac_capturer::PeerConnectionInterface::kBundlePolicyBalanced;
    case RTCBundlePolicyMaxCompat:
      return webrtc_mac_capturer::PeerConnectionInterface::kBundlePolicyMaxCompat;
    case RTCBundlePolicyMaxBundle:
      return webrtc_mac_capturer::PeerConnectionInterface::kBundlePolicyMaxBundle;
  }
}
...
+ (RTCBundlePolicy)bundlePolicyForNativePolicy:
  ...
  switch (nativePolicy) {
    case webrtc_mac_capturer::PeerConnectionInterface::kBundlePolicyBalanced:
      return RTCBundlePolicyBalanced;
    case webrtc_mac_capturer::PeerConnectionInterface::kBundlePolicyMaxCompat:
      return RTCBundlePolicyMaxCompat;
    case webrtc_mac_capturer::PeerConnectionInterface::kBundlePolicyMaxBundle:
...
+ (webrtc_mac_capturer::PeerConnectionInterface::RtcpMuxPolicy)nativeRtcpMuxPolicyForPolicy:
  ...
  switch (policy) {
    case RTCRtcpMuxPolicyNegotiate:
      return webrtc_mac_capturer::PeerConnectionInterface::kRtcpMuxPolicyNegotiate;
    case RTCRtcpMuxPolicyRequire:
      return webrtc_mac_capturer::PeerConnectionInterface::kRtcpMuxPolicyRequire;
  }
}
...
+ (RTCRtcpMuxPolicy)rtcpMuxPolicyForNativePolicy:
  ...
  switch (nativePolicy) {
    case webrtc_mac_capturer::PeerConnectionInterface::kRtcpMuxPolicyNegotiate:
      return RTCRtcpMuxPolicyNegotiate;
    case webrtc_mac_capturer::PeerConnectionInterface::kRtcpMuxPolicyRequire:
...
+ (webrtc_mac_capturer::PeerConnectionInterface::TcpCandidatePolicy)
    nativeTcpCandidatePolicyForPolicy:(RTCTcpCandidatePolicy)policy {
  switch (policy) {
    case RTCTcpCandidatePolicyEnabled:
      return webrtc_mac_capturer::PeerConnectionInterface::kTcpCandidatePolicyEnabled;
    case RTCTcpCandidatePolicyDisabled:
      return webrtc_mac_capturer::PeerConnectionInterface::kTcpCandidatePolicyDisabled;
  }
}

+ (webrtc_mac_capturer::PeerConnectionInterface::CandidateNetworkPolicy)
    nativeCandidateNetworkPolicyForPolicy:(RTCCandidateNetworkPolicy)policy {
  switch (policy) {
    case RTCCandidateNetworkPolicyAll:
      return webrtc_mac_capturer::PeerConnectionInterface::kCandidateNetworkPolicyAll;
    case RTCCandidateNetworkPolicyLowCost:
      return webrtc_mac_capturer::PeerConnectionInterface::kCandidateNetworkPolicyLowCost;
  }
}
...
+ (RTCTcpCandidatePolicy)tcpCandidatePolicyForNativePolicy:
  ...
  switch (nativePolicy) {
    case webrtc_mac_capturer::PeerConnectionInterface::kTcpCandidatePolicyEnabled:
      return RTCTcpCandidatePolicyEnabled;
    case webrtc_mac_capturer::PeerConnectionInterface::kTcpCandidatePolicyDisabled:
...
+ (RTCCandidateNetworkPolicy)candidateNetworkPolicyForNativePolicy:
  ...
  switch (nativePolicy) {
    case webrtc_mac_capturer::PeerConnectionInterface::kCandidateNetworkPolicyAll:
      return RTCCandidateNetworkPolicyAll;
    case webrtc_mac_capturer::PeerConnectionInterface::kCandidateNetworkPolicyLowCost:
...
+ (webrtc_mac_capturer::PeerConnectionInterface::ContinualGatheringPolicy)
    nativeContinualGatheringPolicyForPolicy:
        (RTCContinualGatheringPolicy)policy {
  switch (policy) {
    case RTCContinualGatheringPolicyGatherOnce:
      return webrtc_mac_capturer::PeerConnectionInterface::GATHER_ONCE;
    case RTCContinualGatheringPolicyGatherContinually:
      return webrtc_mac_capturer::PeerConnectionInterface::GATHER_CONTINUALLY;
  }
}
...
+ (RTCContinualGatheringPolicy)continualGatheringPolicyForNativePolicy:
    (webrtc_mac_capturer::PeerConnectionInterface::ContinualGatheringPolicy)nativePolicy {
  switch (nativePolicy) {
    case webrtc_mac_capturer::PeerConnectionInterface::GATHER_ONCE:
      return RTCContinualGatheringPolicyGatherOnce;
    case webrtc_mac_capturer::PeerConnectionInterface::GATHER_CONTINUALLY:
...
+ (webrtc_mac_capturer::SdpSemantics)nativeSdpSemanticsForSdpSemantics:(RTCSdpSemantics)sdpSemantics {
  switch (sdpSemantics) {
    case RTCSdpSemanticsPlanB:
      return webrtc_mac_capturer::SdpSemantics::kPlanB;
    case RTCSdpSemanticsUnifiedPlan:
      return webrtc_mac_capturer::SdpSemantics::kUnifiedPlan;
  }
}

+ (RTCSdpSemantics)sdpSemanticsForNativeSdpSemantics:(webrtc_mac_capturer::SdpSemantics)sdpSemantics {
  switch (sdpSemantics) {
    case webrtc_mac_capturer::SdpSemantics::kPlanB:
      return RTCSdpSemanticsPlanB;
    case webrtc_mac_capturer::SdpSemantics::kUnifiedPlan:
