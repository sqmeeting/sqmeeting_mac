...
class ObjCCallbackVideoSink : public rtc::VideoSinkInterface<webrtc_mac_capturer::VideoFrame> {
 public:
...
@implementation ObjCVideoTrackSourceTests {
  rtc::scoped_refptr<webrtc_mac_capturer::ObjCVideoTrackSource> _video_source;
}

- (void)setUp {
  _video_source = new rtc::RefCountedObject<webrtc_mac_capturer::ObjCVideoTrackSource>();
}
...
- (void)testOnCapturedFrameAdaptsFrame {
  ...
  rtc::VideoSourceInterface<webrtc_mac_capturer::VideoFrame> *video_source_interface = _video_source;
...
- (void)testOnCapturedFrameI420BufferNeedsAdaptation {
  rtc::scoped_refptr<webrtc_mac_capturer::I420Buffer> i420Buffer = CreateI420Gradient(720, 1280);
  ...
  ObjCCallbackVideoSink callback_video_sink(^void(RTC_OBJC_TYPE(RTCVideoFrame) * outputFrame) {
    ...
    XCTAssertEqual(psnr, webrtc_mac_capturer::kPerfectPSNR);
...
- (void)testOnCapturedFrameI420BufferNeedsCropping {
  rtc::scoped_refptr<webrtc_mac_capturer::I420Buffer> i420Buffer = CreateI420Gradient(380, 640);
  RTC_OBJC_TYPE(RTCI420Buffer) *buffer =
      [[RTC_OBJC_TYPE(RTCI420Buffer) alloc] initWithFrameBuffer:i420Buffer];
  RTC_OBJC_TYPE(RTCVideoFrame) *frame =
      [[RTC_OBJC_TYPE(RTCVideoFrame) alloc] initWithBuffer:buffer
