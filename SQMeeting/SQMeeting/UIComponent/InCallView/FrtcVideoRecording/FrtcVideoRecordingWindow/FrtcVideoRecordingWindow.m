...
@interface FrtcVideoRecordingWindow () <NSTextFieldDelegate>
...
@implementation FrtcVideoRecordingWindow
...
- (void)onOKBtnPressed {
    if(self.startVideoRecordingDelegate && [self.startVideoRecordingDelegate respondsToSelector:@selector(startVideoRecording)]) {
        [self.startVideoRecordingDelegate startVideoRecording];
    }
...
- (void)onCancelBtnPressed {
    if(self.startVideoRecordingDelegate && [self.startVideoRecordingDelegate respondsToSelector:@selector(cancelVideoRecording)]) {
        [self.startVideoRecordingDelegate cancelVideoRecording];
    }
...
- (NSTextField *)titleTextField {
    if (!_titleTextField){
        ...
        _titleTextField.stringValue = NSLocalizedString(@"FM_VIDEO_START_RECORDING", @"Start Cloud Recording？");
...
- (NSTextField *)descriptionTextField {
    if (!_descriptionTextField){
        ...
        _descriptionTextField.stringValue = NSLocalizedString(@"FM_VIDEO_START_RECORDING_MESSAGE", @"It will record meeting audio, video and shared screen, and inform all members.");
...
- (FrtcMultiTypesButton *)okButton {
    if (!_okButton){
        _okButton = [[FrtcMultiTypesButton alloc] initFirstWithFrame:CGRectMake(0, 0, 158, 36) withTitle:NSLocalizedString(@"FM_VIDEO_START_RECORDING_BUTTON_TITLE", @"Start")];
        _okButton.target = self;
        _okButton.layer.cornerRadius = 4.0;
        NSMutableAttributedString *attrTitle = [[NSMutableAttributedString alloc] initWithString:NSLocalizedString(@"FM_VIDEO_START_RECORDING_BUTTON_TITLE", @"Start")];
