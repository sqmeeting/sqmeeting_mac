...
@interface FrtcVideoStreamingEndWindow () <NSTextFieldDelegate>
...
@implementation FrtcVideoStreamingEndWindow
...
- (void)onOKBtnPressed {
    if(self.endWindowDelegate && [self.endWindowDelegate respondsToSelector:@selector(endVideoStreaming)]) {
        [self.endWindowDelegate endVideoStreaming];
    }
...
- (void)onCancelBtnPressed {
    if(self.endWindowDelegate && [self.endWindowDelegate respondsToSelector:@selector(cancelEndingVideoStreaming)]) {
        [self.endWindowDelegate cancelEndingVideoStreaming];
    }
...
- (NSTextField *)descriptionTextField {
    if (!_descriptionTextField){
        ...
        _descriptionTextField.stringValue = NSLocalizedString(@"FM_VIDEO_STREAMING_END_DESCRIPTION", @"Audiences are watching the live, are you sure to end the live?");
...
- (FrtcMultiTypesButton *)okButton {
    if (!_okButton) {
        _okButton = [[FrtcMultiTypesButton alloc] initFirstWithFrame:CGRectMake(0, 0, 158, 36) withTitle:NSLocalizedString(@"FM_VIDEO_STREAMING_END_BUTTON_TITLE", @"End")];
        _okButton.target = self;
        _okButton.layer.cornerRadius = 4.0;
        NSMutableAttributedString *attrTitle = [[NSMutableAttributedString alloc] initWithString:NSLocalizedString(@"FM_VIDEO_STREAMING_END_BUTTON_TITLE", @"End")];
