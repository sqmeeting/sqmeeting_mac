...
@interface FrtcVideoStreamingMuteTipsWindow ()
...
@implementation FrtcVideoStreamingMuteTipsWindow
...
- (void)onOKBtnPressed {
    if(self.reminderWindowDelegate && [self.reminderWindowDelegate respondsToSelector:@selector(startStreamingAnyway)]) {
        [self.reminderWindowDelegate startStreamingAnyway];
    }
...
- (void)onCancelBtnPressed {
    if(self.reminderWindowDelegate && [self.reminderWindowDelegate respondsToSelector:@selector(unMuteStreaming)]) {
        [self.reminderWindowDelegate unMuteStreaming];
    }
...
- (NSTextField *)titleTextField {
    if (!_titleTextField){
        ...
        _titleTextField.stringValue = NSLocalizedString(@"FM_VIDEO_STREAMING_MUTE_REMINDER", @"You are muted");
...
- (NSTextField *)descriptionTextField {
    if (!_descriptionTextField){
        ...
        _descriptionTextField.stringValue = NSLocalizedString(@"FM_VIDEO_STREAMING_MUTE_DESCRIPTION", @"Your live streaming will not live audio");
...
- (FrtcMultiTypesButton *)okButton {
    if (!_okButton){
        _okButton = [[FrtcMultiTypesButton alloc] initFirstWithFrame:CGRectMake(0, 0, 158, 36) withTitle:NSLocalizedString(@"FM_VIDEO_STREAMING_BUTTON_MUTE_RECORDING", @"Live without Audio")];
        _okButton.target = self;
        _okButton.layer.cornerRadius = 4.0;
        NSMutableAttributedString *attrTitle = [[NSMutableAttributedString alloc] initWithString:NSLocalizedString(@"FM_VIDEO_STREAMING_BUTTON_MUTE_RECORDING", @"Live without Audio")];
...
- (FrtcMultiTypesButton*)cancelButton {
    if (!_cancelButton){
        _cancelButton = [[FrtcMultiTypesButton alloc] initSecondaryWithFrame:CGRectMake(0, 0, 158, 36) withTitle:NSLocalizedString(@"FM_VIDEO_STREAMING_BUTTON_UNMUTE", @"Unmute")];
        ...
        NSMutableAttributedString *attrTitle = [[NSMutableAttributedString alloc] initWithString:NSLocalizedString(@"FM_VIDEO_STREAMING_BUTTON_UNMUTE", @"Unmute")];
        NSUInteger len = [attrTitle length];
        NSRange range = NSMakeRange(0, len);
        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
        paragraphStyle.alignment = NSTextAlignmentCenter;
